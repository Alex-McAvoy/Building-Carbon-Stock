...
$(function () {
    if (USER.authority == 0) {
        getDisplayDieselFuelConsumption(1);
...
//ajax获取数据展示分页数据
function getDisplayDieselFuelConsumption(pn) {
    $.ajax({
        type: "GET",
        url: APP_PATH + "/dieselFuel-consumption/getDieselFuelConsumption",
        data: "pn=" + pn,
        success: function (result) {
            showDisplayDieselFuelTable(result.extend.pageInfo);
        }
    });
}

//生成数据展示表格
function showDisplayDieselFuelTable(pageInfo) {
    ...
    displayDieselFuelTablePage(total, pageNum, pageSize); //开启分页组件
}

//启用数据展示表格 layui 分页
function displayDieselFuelTablePage(total, pageNum, pageSize) {
    layui.use(["laypage", "jquery"], function () {
        var laypage = layui.laypage;
        var $ = layui.jquery;

        laypage.render({
            elem: "displayDieselFuelTablePage", //id，不用加 #
            count: total, //数据总数
            ...
            jump: function (obj, first) { //点击页号时
                ...
                if (!first) { //首次不执行，点击时才执行
                    getDisplayDieselFuelConsumption(obj.curr); //执行查询分页函数
                }
            }
        });
    });
